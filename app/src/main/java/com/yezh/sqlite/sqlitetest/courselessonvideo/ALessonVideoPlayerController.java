 ...
 * 控制器抽象类
 */
public abstract class ALessonVideoPlayerController
        extends FrameLayout implements View.OnTouchListener {

    private Context mContext;
    protected ILessonVideoPlayer mVideoPlayer;
    ...
    public ALessonVideoPlayerController(Context context) {
        super(context);
        mContext = context;
     ...
     * @param catalogueList
     */
    public abstract void setCatalogueList(List<VideoInfo> catalogueList);
     ...
     *                  <ul>
     *                  <li>{@link LessonVideoPlayer#STATE_PREPARING}</li>
     *                  <li>{@link LessonVideoPlayer#STATE_PREPARED}</li>
     *                  <li>{@link LessonVideoPlayer#STATE_PLAYING}</li>
     *                  <li>{@link LessonVideoPlayer#STATE_PAUSED}</li>
     *                  <li>{@link LessonVideoPlayer#STATE_BUFFERING_PLAYING}</li>
     *                  <li>{@link LessonVideoPlayer#STATE_BUFFERING_PAUSED}</li>
     *                  <li>{@link LessonVideoPlayer#STATE_ERROR}</li>
     *                  <li>{@link LessonVideoPlayer#STATE_COMPLETED}</li>
     *                  </ul>
     */
    protected abstract void onPlayStateChanged(int playState);

    protected abstract void onPlayModeChanged();
    ...
    protected void startUpdateProgressTimer() {
        ...
        if (mUpdateProgressTimerTask == null) {
            mUpdateProgressTimerTask = new TimerTask() {
                @Override
                public void run() {
                    ALessonVideoPlayerController.this.post(new Runnable() {
                        @Override
                        public void run() {
                            updateProgress();
                        }
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (!mNeedChangePosition && !mNeedChangeVolume && !mNeedChangeBrightness) {
                    ...
                    if (absDeltaX >= THRESHOLD) {
                        ...
                        if (mDownX < getWidth() * 0.5f) {
                            ...
                            mGestureDownBrightness = LessonUtil.scanForActivity(mContext)
                ...
                if (mNeedChangeBrightness) {
                    ...
                    WindowManager.LayoutParams params = LessonUtil.scanForActivity(mContext)
                            .getWindow().getAttributes();
                    params.screenBrightness = newBrightnessPercentage;
                    LessonUtil.scanForActivity(mContext).getWindow().setAttributes(params);
